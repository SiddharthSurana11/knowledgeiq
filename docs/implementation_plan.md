# Implementation Plan: KnowledgeIQ Platform Design

This document provides a detailed technical implementation roadmap for the **KnowledgeIQ** document intelligence platform.
...
1.  **[MODIFY] [server.js](../apps/api-gateway/server.js)**:
    *   Remove static folder resolution case statement.
    *   Implement database lookup: Query `categories` collection in MongoDB by `category` (slug key) to fetch folder settings dynamically.
    *   Create `GET /api/categories` route returning `_id, key, name, description` from MongoDB.
2.  **[MODIFY] [pineconeClient.js](../apps/api-gateway/utils/pineconeClient.js)**:
...
3.  **[MODIFY] [chatRoute.js](../apps/api-gateway/routes/chatRoute.js)**:
    *   Extract `categoryId` from request body payload.
    *   Pass `categoryId` parameter down to `getRelevantChunks`.
4.  **[MODIFY] [vector_db_pinecone.py](../apps/embedding-service/modules/vectorDB/vector_db_pinecone.py)**:
    *   Delete string matching `detect_category(query_text)` function.
5.  **[MODIFY] [UploadPanel.jsx](../apps/frontend_reactjs/src/components/UploadPanel.jsx)**:
...
6.  **[MODIFY] [prompt_instructions.txt](../apps/llm-service/prompt_instructions.txt)**:
    *   Generalize prompt: Replace domain-specific prompt text with "You are KnowledgeIQ, a corporate smart assistant trained on internal documentation."

#### Exact Logic to Add
*   **Database Seeding Script**: Create a startup seed in MongoDB to populate initial category templates (`hr`, `finance`, `compliance`, `other`).
...
#### Exact Files to Modify
1.  **[MODIFY] [app.py](../apps/embedding-service/app.py)**:
...
2.  **[MODIFY] [vector_db_pinecone.py](../apps/embedding-service/modules/vectorDB/vector_db_pinecone.py)**:
...
3.  **[MODIFY] [server.js](../apps/api-gateway/server.js)**:
...
4.  **[MODIFY] [AdminPanel.jsx](../apps/frontend_reactjs/src/components/AdminPanel.jsx)**:
...
#### Exact Files to Modify
1.  **[MODIFY] [llm_service.proto](../protos/llm_service.proto)**:
    *   Add the `CheckContradiction` RPC endpoint and associated Request/Response messages.
2.  **[MODIFY] [app.py](../apps/llm-service/app.py)** (LLM Service):
    *   Expose the `CheckContradiction` gRPC method in `LLMServiceServicer`.
3.  **[MODIFY] [response_generator.py](../apps/llm-service/response_generator.py)**:
...
4.  **[MODIFY] [llmClient.js](../apps/api-gateway/utils/llmClient.js)**:
...
5.  **[MODIFY] [app.py](../apps/embedding-service/app.py)** (Embedding Service):
...
#### Exact Files to Modify
1.  **[MODIFY] [app.py](../apps/llm-service/app.py)**:
    *   Extend `BackgroundScheduler` in the LLM service to run a daily task: `run_freshness_audit()`.
2.  **[MODIFY] [embedder.py](../apps/embedding-service/modules/vectorDB/embedder.py)**:
...
2.  **[MODIFY] [App.jsx](../apps/frontend_reactjs/src/App.jsx)**:
    *   Add a tab state toggle to switch between the chat interface and the administrative dashboard.
3.  **[MODIFY] [AdminPanel.jsx](../apps/frontend_reactjs/src/components/AdminPanel.jsx)** (Refactor):
